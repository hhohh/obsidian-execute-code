...
export class Outputter extends EventEmitter {
	...
	/** The absolute start line of the section containing this block */
	private sectionStartLine: number;

	/** The index of the code block within its section */
	private blockIndex: number;

	/** Post-processor context for identifying block location */
	private context: MarkdownPostProcessorContext | undefined;
	/** Source code of this SQL block, captured at creation time for reliable matching */
	private sqlBlockSourceCode: string;
	constructor(
	...
	/**
	 * Finalize SQL output: parse the accumulated buffer, persist to file,
	 * and let the vtable code block processor handle rendering.
	...
	finalizeSqlOutput() {
		...
		if (parsed && parsed.columns.length > 0 && parsed.records.length > 0) {
			// 持久化到文件，由 Obsidian 渲染 vtable 代码块处理器负责显示表格
	...
	private async findCodeBlockEndLineByIndex(): Promise<number> {
		if (!this.sqlBlockSourceCode) return -1;

		// 优先从编辑器缓冲区获取内容（处理未保存的修改）
		const activeView = this.app.workspace.getActiveViewOfType(MarkdownView);
		if (activeView && activeView.file && activeView.file.path === this.srcFile) {
			return this.findCodeBlockEndLineInContent(activeView.editor.getValue());
		}
		...
		return this.findCodeBlockEndLineInContent(content);
	}

	/**
	 * Find the end line of the SQL block whose source code matches this.sqlBlockSourceCode.
	 * Uses source code matching to reliably locate the correct block, regardless
	 * of whether the post-processor processed sections independently.
	 */
	private findCodeBlockEndLineInContent(content: string): number {
		const lines = content.split('\n');
		const targetSource = this.sqlBlockSourceCode.trim();
		...
		return bestMatch !== -1 ? bestMatch : firstMatch;
	}
	...
	private async persistSqlResultToFile(data: SqlResultData) {
		...
		try {
			...
			if (insertLine === -1) {
				console.error('[Execute Code] Cannot locate code block end line by index');
				return;
			}
			...
			if (isSameFile) {
				...
				// 检查是否已存在 vtable 块（紧随代码块之后）
				let nextLineIdx = insertLine + 1;
				while (nextLineIdx < editor.lineCount() && editor.getLine(nextLineIdx).trim() === "") {
					nextLineIdx++;
				}
				const hasExistingVTable = nextLineIdx < editor.lineCount() &&
					editor.getLine(nextLineIdx).trim() === '```vtable';
				if (hasExistingVTable) {
					// 定位 vtable 块的结束行
					let endLine = nextLineIdx;
					while (endLine < editor.lineCount()) {
						if (editor.getLine(endLine).trim() === '```' && endLine > nextLineIdx) break;
						endLine++;
					}
					editor.replaceRange(vtableBlock.trim(),
						{ line: nextLineIdx, ch: 0 },
						{ line: endLine, ch: 3 });
				} else {
					const currentLineContent = editor.getLine(insertLine);
					editor.replaceRange(vtableBlock,
						{ line: insertLine, ch: currentLineContent.length });
				}
				return;
			}
